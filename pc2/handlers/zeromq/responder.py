...
from pc2.util.parsing import s2b, b2s, ia2s, sa2s, m2s
import xarray as xa
...
class PC2ZMQResponder(Thread):
    ...
    def getKeyDir( self, **kwargs ):
        base_dir = kwargs.get( "certificate_path", os.path.expanduser("~/.pc2/") )
    ...
    def processWorkflows(self, workflows: Dict[str, PC2Workflow]) -> List[str]:
        completed_requests = []
...
#        self.logger.info(f"@@SR: --> Set Execution Status[{rid}]: {str(status)}")
        try:
            if status == Status.EXECUTING:
                self.executing_jobs[rid] = PC2Response(rid, {"status": "executing"})
