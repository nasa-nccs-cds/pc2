...
class WPSExecuteRequest:
    ...
    def _getPC2RequestStr( self ): return '%s?request=Execute&service=WPS&status=true&version=1.0.0' % ( self._host_address )
    ...
    def _getPC2RequestDict( self ): return dict( request='Execute', service='WPS', status='true', version='1.0.0'  )
    ...
    def getWpsStr( self, requestSpec: Dict ) -> str:
        return self._getPC2RequestStr() + self._toDatainputsStr( requestSpec )

    def getWpsParms( self, requestSpec: Dict ) -> Dict:
        return { **self._getPC2RequestDict(), **self._toDatainputsDict( requestSpec ) }
