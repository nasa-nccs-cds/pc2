...
class PC2CoreBase:
    ...
    @abc.abstractmethod
    def getClients( self, op: Op = None, **kwargs ) -> List[PC2Client]: pass

    @abc.abstractmethod
    def getClient( self, **kwargs ) -> PC2Client: pass

    @abc.abstractmethod
    def getApplication( self ) -> "PC2AppBase": pass
...
class PC2AppBase(Thread):
    ...
    def run(self):
        self.logger.info(" &&&&&&&&&&&&&&&&&&&&&&&&& Running PC2 App: " + self.__class__.__name__ + " &&&&&&&&&&&&&&&&&&&&&&&&&")
        self.initInteractions()
        self.logger.info(" &&&&&&&&&&&&&&&&&&&&&&&&& Starting PC2 App Loop &&&&&&&&&&&&&&&&&&&&&&&&&")
        while self._active:
            self.ingestRequests()
    ...
    def ingestRequests( self ):
        ...
        while True:
            try:
                ...
                workflow = PC2Workflow(error=(msg, err))
    ...
    def getWorkflowItems(self) -> ItemsView[str, PC2Workflow]:
...
class PC2ServerApp(PC2AppBase):
    __metaclass__ = abc.ABCMeta

    def __init__( self, core: PC2CoreBase, **kwargs ):
        PC2AppBase.__init__( self, core, **kwargs )
...
class PC2EmbeddedApp(PC2AppBase):
...
class PC2Factory:
    ...
    @abc.abstractmethod
    def client( self, core: PC2CoreBase, **kwargs ) -> PC2Client: pass

    @abc.abstractmethod
    def getClient( self, cid: str = None ) -> PC2Client: pass

    @abc.abstractmethod
    def app(self, core: PC2CoreBase ) -> PC2AppBase: pass
